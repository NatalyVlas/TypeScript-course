...
export function toggleFavoriteItem(place: FavouritePlace): void {
  ...
  const localStorageFavouriteItems: FavouritePlace[] = JSON.parse(localStorage.getItem("favoriteItems"));
  if (!localStorageFavouriteItems) {
    ...
    if (
      ...
      localStorage.setItem("favoriteItems",
        JSON.stringify(localStorageFavouriteItems.filter((item) => item.id !== place.id))
      );
      favoritesElements.forEach((el: HTMLElement) => {
        if (+el.dataset.id === +place.id) {
        ...
        <div class="result-container">
          ...
          <div class="result-info">
            ...
            <div class="result-info--footer">
              <div>
                <button class="book-btn" data-id=${result.id}>Забронировать</button>
              </div>
            </div>
          </div>
        </div>
